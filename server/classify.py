...
class classifier(model):
    def __init__(self):
        super().__init__()
        self.model = 'inception-resnet-v2-tf.xml'
        self.device = 'MYRIAD'
        self.number_top = 10
    ...
    def load_model(self):
        ...
        assert self.net.input_info["input"].input_data.shape[0] == 1, "Sample supports only single input topologies"
        ...
        #load classes
        with open("imagenet_class_index.txt",'r') as file:
            self.labels_map=file.readlines()
    ...
    def classify(self, image):
        ...
        n, c, h, w = self.net.input_info["input"].input_data.shape
        ...
        return ["{}{}\t{}{}{:.7f}".format(' ' * space_num_before, self.labels_map[top_ind[0]] if self.labels_map else "{}".format(top_ind[0]),
        ' ' * space_num_after, ' ' * space_num_before_prob,
        probs[top_ind[0]]),
        "{}{}\t{}{}{:.7f}".format(' ' * space_num_before, self.labels_map[top_ind[1]] if self.labels_map else "{}".format(top_ind[0]),
        ' ' * space_num_after, ' ' * space_num_before_prob,
        probs[top_ind[1]]),
        "{}{}\t{}{}{:.7f}".format(' ' * space_num_before, self.labels_map[top_ind[2]] if self.labels_map else "{}".format(top_ind[0]),
        ' ' * space_num_after, ' ' * space_num_before_prob,
        probs[top_ind[2]]),
        "{}{}\t{}{}{:.7f}".format(' ' * space_num_before, self.labels_map[top_ind[3]] if self.labels_map else "{}".format(top_ind[0]),
        ' ' * space_num_after, ' ' * space_num_before_prob,
        probs[top_ind[3]]),
        "{}{}\t{}{}{:.7f}".format(' ' * space_num_before, self.labels_map[top_ind[4]] if self.labels_map else "{}".format(top_ind[0]),
        ' ' * space_num_after, ' ' * space_num_before_prob,
        probs[top_ind[4]])
        ]
